                ...
                <div className="w-full h-full grid grid-cols-4 gap-4">
                    {
                        fileEntry.files.slice(0,(fileEntry.files.length<4?fileEntry.files.length:4)).map((image,index)=>{
                            return(
                                <div key={image.uuid} className="w-full h-full relative">
                                    <img className="w-full h-full rounded-md z-10"  src={`${image.cdnUrl}-/preview/600x800/-/format/auto/-/quality/smart/` || ''} alt={image.name}/>
                                    
                                    {
                                        index===3?
                                        <div onClick={()=>{setOpen(true)}} className={`${fileEntry.files.length>4?"block":"hidden"} w-full h-full absolute inset-0 rounded-md bg-zinc-700/90 flex flex-col justify-center items-center font-mono font-semibold text-base text-white hover:cursor-pointer`}>
                                            {`+ ${fileEntry.files.length-4}`}
                                        </div>:
                                        <div onClick={() => handleRemoveClick(image.uuid)} className="absolute z-20 -top-2 -right-2 w-4 h-4 flex flex-col bg-white rounded-full justify-center items-center hover:cursor-pointer">
